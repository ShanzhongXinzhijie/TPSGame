...
void GuiList::Draw() {
	back.Draw(position, backscale, pivot, 0, color, DirectX::SpriteEffects_None, depth);
	...
	for (const std::wstring& v : values) {
		font.Draw(v.c_str(), valuePos, color, fontscale, CVector2::Zero(), 0.0f, DirectX::SpriteEffects_None, depth);

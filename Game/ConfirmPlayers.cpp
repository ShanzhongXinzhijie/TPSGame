...
void ConfirmPlayers::Update() {
	...
	if (Pad(0).GetDown(enButtonStart)) {
		...
		if (isJoin && fade->isIdel()) {
			...
			fade->fadeIn([&]() {
				bgm->Stop();
				Game* game = new Game(fade,m_timeLimit,m_citizenCnt, seed, -1);
				game->SetName(L"GameClass");
				delete this; return;
			});
#else
			//Ready!
	...
	if (GetPhoton()->GetIsMasterClient()) {
		...
		if (start) {
			...
			fade->fadeIn([&]() {
				bgm->Stop();
				Game* game = new Game(fade, m_timeLimit, m_citizenCnt, m_seed, m_startTime);
				game->SetName(L"GameClass");
				delete this; return;
			});
		}
		...
		fade->fadeIn([&]() {
			bgm->Stop();
			Game* game = new Game(fade, m_timeLimit, m_citizenCnt, m_netWork->GetNetEventReceiver().GetGameStartData().m_seed, m_netWork->GetNetEventReceiver().GetGameStartData().m_startTime);
			game->SetName(L"GameClass");
			delete this; return;
		});
	}
